...
static ConVar cl_logfps( "cl_logfps", "0", 0, "Log current position to the console." );
...
void CFPSPanel::Paint() 
{
	...
	if ( cl_logfps.GetInt() > 0 ) {
		C_BasePlayer * pPlayer = C_BasePlayer::GetLocalPlayer();
		Vector vector = MainViewOrigin();
		char mapname[MAX_MAP_NAME];
	    Q_FileBase( engine->GetLevelName(), mapname, sizeof(mapname) );
		if ( pPlayer ) {
			string_t name = pPlayer->GetPlayerName();
			vector = pPlayer->EyePosition();
			Msg( "PosLog, %s, %s, %.02f, %.02f, %.02f, %.02f\n", mapname, name, gpGlobals->realtime, vector.x, vector.y, vector.z );
		}
	}
}
